...
v4sf value(float l[2], struct ray ray)
{
	...
	while (l[0] < l[1]) {
		...
		if (l[0] < x0 && x0 < (l[0] + 0.1) && l[0] < x1 && x1 < (l[0] + 0.1)) {
			if (a * curve(ray.o + v4sf_set1(x0) * ray.d) <= 0) {
				l[1] = x0;
				break;
			}
			if (a * curve(ray.o + v4sf_set1(x1) * ray.d) <= 0) {
				l[1] = x1;
				break;
			}
			if (a * curve(ray.o + v4sf_set1(0.5 * (x0 + x1)) * ray.d) <= 0) {
				l[1] = 0.5 * (x0 + x1);
				break;
			}
		}
